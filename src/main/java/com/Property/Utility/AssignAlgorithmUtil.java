...
public class AssignAlgorithmUtil
{
	...
	public static void taskAllocation(int task_num,int rev_num,String[][] rev_task)
	{
		...
		{//第二种情况：第一轮可容纳的任务数大于待分配的任务数，此时降低一个单位的标杆(max_task-1)，然后循环计算可容纳的任务数，直到退出循环
			...
			if(ava_task-task_num>0)
			{//如果可容纳的任务数大于待分配的任务数，那么需要再再降低一个单位的标杆
				max_task--;
				ava_task=0;
				lower_List.removeAll(lower_List);
				for(int i=0;i<rev_num;i++)
		...
		for(int i=0;i<rev_num;i++)
		{
			int param1=Integer.parseInt(rev_task[i][1]);
			int param2=Integer.parseInt(rev_task[i][2]);
			rev_task[i][1]=String.valueOf(param1+param2);
		}
	}
